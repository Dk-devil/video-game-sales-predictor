...
def predict_global_sales(Platform,Year_of_Release,Genre,Publisher,
    ...
    # Now you can manipulate the CSR matrix to drop columns
    columns_to_drop = [2335]  # Example: Columns you want to drop

    # Drop specified columns
    sparse_matrix_csr = sparse_matrix_csr[:, [col for col in range(sparse_matrix_csr.shape[1]) if col not in columns_to_drop]]

    # If you want to convert back to COO format, you can do so
...
def main():
    ...
    Genre = st.selectbox("Genre", sorted(list(dataset['Genre'].astype(str).unique())))
    Publisher = st.selectbox("Publisher", sorted(list(dataset['Publisher'].astype(str).unique())))
    ...
    Developer = st.selectbox("Developer", sorted(list(dataset['Developer'].astype(str).unique())))
    Rating = st.selectbox("Select Rating", sorted(list(dataset['Rating'].astype(str).unique())))
